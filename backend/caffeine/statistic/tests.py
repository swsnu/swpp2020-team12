...
class StatisticTestCase(TestCase):
    def setUp(self):  # beforeeach 같은거
        ...
        dailystudyrecord1 = DailyStudyRecord.objects.create(user=user1, total_study_time=timedelta(seconds=1), date=datetime(2020, 11, 25), total_concentration=timedelta(0))
        dailystudyforsubject1 = DailyStudyForSubject.objects.create(date=datetime(2020, 11, 25) ,study_time=timedelta(seconds=1),
